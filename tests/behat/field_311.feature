...
Feature: Managers can manage course custom fields textregex 311
  ...
  Scenario: Create a custom course textregex field 311
  ...
  Scenario: Edit a custom course textregex field 311
    When I click on "Add a new custom field" "link"
    And I click on "Short text with regex validation" "link"
    And I set the following fields to these values:
      | Name               | Test field |
  ...
  Scenario: Delete a custom course textregex field 311
    When I click on "Add a new custom field" "link"
    And I click on "Short text with regex validation" "link"
    And I set the following fields to these values:
      | Name               | Test field |
  ...
  Scenario: A textregex field must validate it on course edit form 311
  ...
  Scenario: A textregex field with a default value must be shown on listing but allow empty values that will not be shown 311
    Given the following "users" exist:
      | username | firstname | lastname  | email                |
      | teacher1 | Teacher   | Example 1 | teacher1@example.com |
    And the following "courses" exist:
